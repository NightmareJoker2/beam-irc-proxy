...
exports.extend = function (server) {
	...
	server.channels = new EventManager({
		...
		function (client, channel) {
			...
			BeamClient.request('GET', 'channels/' + beamChannel)
				...
				.then(response => {
					...
					console.log(body);
					if (!server.channels.list[channel].hasOwnProperty('beamSocket'))
					{
			...
			{
				// Chat connection
				server.channels.list[channel].beamUsers = [];
				server.channels.list[channel].beamSocket = new Beam.Socket(ws, endpoints).boot();
				const socket = server.channels.list[channel].beamSocket;
				...
				socket.on('UserJoin', data => {
					...
					var beamUser = { name: data.username, hostname: data.id + '@' + data.id + '.chat.mixer.com' };
					if (beamUser.name.toUpperCase() != client.name.toUpperCase())
					{
						beamUser.operator = (data.roles.indexOf('Mod') != -1 || data.roles.indexOf('GlobalMod') != -1 || data.roles.indexOf('Owner') != -1 || data.roles.indexOf('Staff') != -1);
						...
						server.channels.list[channel].beamUsers[data.id] = beamUser;
					}
				});
				...
				socket.on('UserLeave', data => {
					...
					if (beamUser.name.toUpperCase() != client.name.toUpperCase())
					{
						...
						if(server.channels.list[channel].beamUsers.hasOwnProperty(data.id))
						{
							//delete server.channels.list[channel].users[data.username];
							if (server.channels.list[channel].beamUsers[data.id].operator)
							{
								server.channels.negotiate('deliver', channel, beamChannel, 'MODE', [channel, '-o ' + data.username], true);
							}
							delete server.channels.list[channel].beamUsers[data.id];
						}
					}
				});
				...
				socket.on('ChatMessage', data => {											
					...
					messageText = messageText.replace(/\r/g, "\u240D").replace(/\n/g, "\u240A"); // see: https://github.com/ircv3/ircv3-specifications/issues/208#issuecomment-179645102 and https://en.wikipedia.org/wiki/Newline#Unicode
					console.log(data.user_name, ': ', messageText);
					var beamUser = { name: data.user_name, hostname:  data.user_id + '@' + data.user_id + '.chat.mixer.com' };
					
					beamUser.operator = (data.user_roles.indexOf('Mod') != -1 || data.user_roles.indexOf('GlobalMod') != -1 || data.user_roles.indexOf('Owner') != -1 || data.user_roles.indexOf('Staff') != -1);
					if (server.channels.list[channel].beamUsers.hasOwnProperty(data.user_id) && server.channels.list[channel].beamUsers[data.user_id].operator != beamUser.operator)
					{
						server.channels.list[channel].beamUsers[data.user_id].operator = beamUser.operator;
						server.channels.negotiate('deliver', channel, beamChannel, 'MODE', [channel, (server.channels.list[channel].beamUsers[data.user_id].operator == true ? '+' : '-') + 'o ' + data.user_name], true);
					}

					if (server.channels.list[channel].beamUsers.hasOwnProperty(data.user_id) && server.channels.list[channel].beamUsers[data.user_id].name != beamUser.name)
					{
						server.channels.list[channel].beamUsers[data.user_id].name = beamUser.name;
					}

					if (!server.channels.list[channel].beamUsers.hasOwnProperty(data.user_id))
					{
						server.channels.negotiate('deliver', channel, beamUser, 'JOIN', [channel]);
						if (beamUser.operator)
						{
						...
						server.channels.list[channel].beamUsers[data.user_id] = beamUser;
				...
				socket.on('connect', data =>
				{
					...
					for (var i = 0, len = server.channels.list[channel].beamUsers.length; i < len; i++)
					{
						server.channels.negotiate('deliver', channel, server.channels.list[channel].beamUsers[i], 'JOIN', [channel]);
					}
					...
					while (true)
					{
						...
						.then(response => {
							...
							{
								var beamUser = { name: chatUsers[i].userName, hostname: chatUsers[i].userId + '@' + chatUsers[i].userId + '.chat.mixer.com' };
								beamUser.operator = (chatUsers[i].userRoles.indexOf('Mod') != -1 || data.user_roles.indexOf('GlobalMod') != -1 || chatUsers[i].userRoles.indexOf('Owner') != -1 || data.userRoles.indexOf('Staff') != -1);
								server.channels.list[channel].beamUsers[chatUsers[i].userId] = beamUser;
								server.channels.negotiate('deliver', channel, beamUser, 'JOIN', [channel]);
								if (beamUser.operator)
								{
									server.channels.negotiate('deliver', channel, beamChannel, 'MODE', [channel, '+o ' + data.username], true);
		...
		'destroy': function (channel) {
			...
			server.channels.list[channel].beamSocket.close();
